...
export type Layout = Component & {
   ...
   name: string; // The custom name of the layout
...
export function getBlockComponent(blockName: string): Block | undefined {
   const block = pageBuilderBlocks.value[toCamelCase(blockName)];
   if (!block) return undefined;
   return block;
}

export function getBlockFields(blockName: string): any {
   try {
...
function initialiseBlockRegistry(): void {
   ...
   Object.entries(blockModules).forEach(([path, module]) => {
      ...
      if (component && component.name) {
         const blockName = toCamelCase(component.name);
         ...
         const block: Block = {
            fields: getBlockFields(blockName),
         ...
         // Convert component name to camelCase for the registry key
         pageBuilderBlocks.value[block.name] = block;
      }
   });
}
